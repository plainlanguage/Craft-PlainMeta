...
class PlainMetaService extends BaseApplicationComponent
{
	...
	public function savePlainMeta(BaseFieldType $fieldType)
	{
		...
		$plainMetaRecord = PlainMeta_PlainMetaRecord::model()->findByAttributes(array(
			'elementId' => $elementId,
			'handle'    => $handle,
		));
...
//			'socialOGVideoOtherReleaseDate',
	 ...
	 *
	 * @param BaseFieldType
	 * @return PlainMeta_PlainMetaModel
	 */
	public function getPlainMeta(BaseFieldType $fieldType)
	{
		$plainMetaRecord = PlainMeta_PlainMetaRecord::model()->findByAttributes(array(
			'elementId' => $fieldType->element->id,
			'handle'    => $fieldType->model->handle,
		...
		if ($plainMetaRecord)
		{
			$attr = $plainMetaRecord->getAttributes();
		} else
		{
			$attr = array();
		}
	 ...
	 * @return PlainMeta_PlainMetaModel
	 */
	public function outputPlainMeta($entry, $fallback)
	{
		// Check for valid entry
		if (isset($entry->id))
		{
			$plainMetaEntry = PlainMeta_PlainMetaRecord::model()->findByAttributes(array(
				'elementId' => $entry->id,
			));
		}
		else
		{
			$plainMetaEntry = false;
		}

		// Check for valid fallback
		if (isset($fallback->id))
		{
			$plainMetaFallback = PlainMeta_PlainMetaRecord::model()->findByAttributes(array(
				'elementId' => $fallback->id,
			));
		}
		else
		{
			$plainMetaFallback = false;
		}

		// Return false if we don't have an entry or fallback record
		if (!$plainMetaEntry && !$plainMetaFallback)
		{
			return false;
		}
		...
		{
			// Entry attributes
			$entryAttributes    = $plainMetaEntry->getAttributes();
			// Fallback attributes
			$fallbackAttributes = $plainMetaFallback ? $plainMetaFallback->getAttributes() : false;
		...
		elseif ($plainMetaFallback)
		{
			$attributes = $plainMetaFallback->getAttributes();
		}
		...
		if ($attributes)
		{
			...
			$newPath = craft()->path->getPluginsPath() . 'plainmeta/templates/';
